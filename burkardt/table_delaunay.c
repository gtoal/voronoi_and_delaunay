...
void vbedg(double x, double y, int point_num, double point_xy[], int tri_num,
           ...
           int *redg);
void print_points(int m, int n, double a[]);
...
//
{
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "TABLE_DELAUNAY\n");
  fprintf(stderr, "  C++ version:\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "  Compiled on %s at %s.\n", __DATE__, __TIME__);
  fprintf(stderr, "\n");
  fprintf(stderr, "  Read a TABLE dataset of N points in 2 dimensions,\n");
  fprintf(stderr, "  Compute the Delaunay triangulation.\n");
  fprintf(stderr, "  Write an integer TABLE dataset of the triangulation.\n");
  //
  ...
  if (argc <= 1) {
    fprintf(stderr, "\n");
    fprintf(stderr, "TABLE_DELAUNAY:\n");
    fprintf(stderr, "  Please enter the file prefix.\n");
    size_t xx = 0;
    getline(&prefix, &xx, stdin);  // fix
  ...
  //
  sprintf(node_filename, "%s%s", prefix, "_nodes.txt");
  sprintf(triangle_filename, "%s%s", prefix, "_elements.txt");
  //
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "  Read the header of \"%s\"\n", node_filename);
  fprintf(stderr, "\n");
  fprintf(stderr, "  Node dimension NODE_DIM = %d\n", node_dim);
  fprintf(stderr, "  Node number    NODE_NUM = %d\n", node_num);

  if (node_dim != 2) {
    fprintf(stderr, "\n");
    fprintf(stderr, "TABLE_DELAUNAY - Fatal error!\n");
    fprintf(stderr, "  The node dimension is not 2.\n");
    exit(1);
  }
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "  Read the data of \"%s\"\n", node_filename);

  print_points(node_dim, node_num, node_xy);
  ...
  triangle_node = calloc(triangle_order * 3 * node_num, sizeof(int));
  triangle_neighbor = calloc(triangle_order * 3 * node_num, sizeof(int));
  ...
  //
  fprintf(stderr, "\n");
  fprintf(stderr, "  Computed the triangulation.\n");
  fprintf(stderr, "  Number of triangles is %d\n", triangle_num);

  i4mat_transpose_print_some(triangle_order, triangle_num, triangle_node, 1, 1,
                             3, 5, "  Initial portion of triangulation data:");
  //
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "  Wrote the triangulation data to \"%s\".\n",
          triangle_filename);
  //
  //  Terminate execution.
  //
...
//
{
  ...
  tola = tol *
         r8_max(fabs(dx10), r8_max(fabs(dy10), r8_max(fabs(dx30), fabs(dy30))));

  tolb = tol *
         r8_max(fabs(dx12), r8_max(fabs(dy12), r8_max(fabs(dx32), fabs(dy32))));
...
//    Output, int DTRIS2, is 0 for no error.
{
  ...
  stack = calloc(point_num, sizeof(int));
  ...
  for (i = 2; i <= point_num; i++) {
    ...
    if (k == -1) {
      fprintf(stderr, "\n");
      fprintf(stderr, "DTRIS2 - Fatal error!\n");
      fprintf(stderr, "  Fails for point number I = %d\n", i);
      fprintf(stderr, "  M =  %d\n", m);
      fprintf(stderr, "  M1 = %d\n", m1);
      fprintf(stderr, "  X,Y(M)  = %f  %f\n", point_xy[2 * (m - 1) + 0],
              point_xy[2 * (m - 1) + 1]);
      fprintf(stderr, "  X,Y(M1) = %f  %f\n", point_xy[2 * (m1 - 1) + 0],
              point_xy[2 * (m1 - 1) + 1]);
      // delete [] stack;
      return 224;
    }
  ...
  *tri_num = j - 2;
  ...
  for (i = j + 1; i <= point_num; i++) {
    ...
    if (error != 0) {
      fprintf(stderr, "\n");
      fprintf(stderr, "DTRIS2 - Fatal error!\n");
      fprintf(stderr, "  Error return from SWAPEC.\n");
      // delete [] stack;
      return error;
    }
  }
...
//  Purpose:
//
//    FILE_COLUMN_COUNT counts the number of columns in the first line of a
//    file.
//
//  Discussion:
//
...
//    separated by spaces.  There may also be some blank lines, and some comment
//    lines, which have a "#" in column 1.
//
...
#define INCX 10
  ...
  for (i2lo = ilo; i2lo <= ihi; i2lo = i2lo + INCX) {
    ...
    for (j = j2lo; j <= j2hi; j++) {
      //
      //  Print out (up to INCX) entries in column J, that lie in the current
      //  strip.
      //
      fprintf(stderr, "%5d,  ", j);
      for (i = i2lo; i <= i2hi; i++)
        fprintf(stderr, "%d, ", a[i - 1 + (j - 1) * m]);
      fprintf(stderr, "\n");
    }
  }
...
//
{
  ...
  for (i = 0; i < n; i++) p[i] = p[i] - p_min + base;
...
//
//    Input, int BASE, is 0 for a 0-based permutation and 1 for a 1-based
//    permutation.
//
...
//
{
  ...
  //  entries of P are strictly positive.  Presumably, the lowest number is
  //  BASE. So temporarily add 1-BASE to each entry to force positivity.
  //
  for (i = 0; i < n; i++) {
    p[i] = p[i] + 1 - base;
  }
...
//
{
  ...
  table = calloc(m * n, sizeof(double));
...
#define INCX 5
  ...
  for (i2lo = i4_max(ilo, 1); i2lo <= i4_min(ihi, m); i2lo = i2lo + INCX) {
    ...
    for (j = j2lo; j <= j2hi; j++) {
      ...
      for (i2 = 1; i2 <= inc; i2++) {
        i = i2lo - 1 + i2;
        fprintf(stderr, "%14f", a[(i - 1) + (j - 1) * m]);
      }
      fprintf(stderr, "\n");
    }
  }
...
#define INCX 5
  ...
  fprintf(stdout, "#define MAX_POINTS %d\n\n", n);

  fprintf(stdout, "typedef struct Point {\n  double xy[2]; // x,y coordinate of this point.  The array of points was the initial input.\n} Point;\n\n");
  // Whoops... angle_to_infinity is only for voronoi diagrams, not delaunay graphs!
  // fprintf(stdout, "typedef struct Node {\n  Point p;\n  int onHull;\n  double angle_to_infinity;\n  int num_edges;\n  int *e; // flexible array of indexes [0:num_edges-1] to edges connected to this point\n} Node;\n\n");
  fprintf(stdout, "typedef struct Node {\n  Point p;\n  int onHull;\n  int num_edges;\n  int *e; // flexible array of indexes [0:num_edges-1] to edges connected to this point\n} Node;\n\n");
  fprintf(stdout, "Node *node = NULL;\n\n");
  fprintf(stdout, "const Point Pt[MAX_POINTS] = {\n"/*, title*/);
  for (i2lo = i4_max(ilo, 1); i2lo <= i4_min(ihi, m); i2lo = i2lo + INCX) {
    i2hi = i2lo + INCX - 1;
    i2hi = i4_min(i2hi, m);
    i2hi = i4_min(i2hi, ihi);
...
//  Purpose:
//
//    S_LEN_TRIM returns the length of a char *to the last nonblank.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
//  Parameters:
//
//    Input, char *S, a string.
//
//    Output, int S_LEN_TRIM, the length of the char *to the last nonblank.
//    If S_LEN_TRIM is 0, then the char *is entirely blank.
//
{
  int n;
...
//  Parameters:
//
//    Input, char *S, the char *containing the
...
//    the char *to form the number, including any terminating
...
//  Parameters:
//
//    Input, char *S, the char *to be read.
//
...
//
{
  ...
  for (i = 0; i < n; i++) {
    rvec[i] = s_to_r8(substr(s, begin, length), &lchar, &error);
...
//  Parameters:
//
//    Input, char *S, the char *to be examined.
//
...
//
{
  ...
  for (;;) {
    ...
    *top = *top - 1;
    ...
    swap = diaedg(x, y, point_xy[2 * (a - 1) + 0], point_xy[2 * (a - 1) + 1],
