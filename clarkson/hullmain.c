...
static void errline(char *s) {fprintf(stderr, "%s\n", s);}
static void tell_options(void) {
	...
	errline("	ps->postscript, mp->metapost, cpr->cpr format, off->OFF format");
	errline("	vn->vertex numbers(default), svg->graphics, png->graphics, txt->plain text, h->header file");
...
char *output_forms[] = {"vn", "ps", "mp", "cpr", "off", "svg", "png", "txt", "h"};

out_func *out_funcs[] = {&vlist_out, &ps_out, &mp_out, &cpr_out, &off_out, &svg_out, &png_out, &txt_out, &h_out};
...
int main(int argc, char **argv) {
	...
	while ((option = getopt(argc, argv, "i:m:rqs:do:X:a:Af:")) != EOF) {
		switch (option) {
		case 'q':
			quiet = 1;
			break;
	...
	if (!quiet) fprintf(DFILE, "reading from %s\n", ifn ? ifile : "stdin");
	...
	if (output) {
		...
		if (!quiet) fprintf(DFILE, "main output to %s\n", ofn ? ofile : "stdout");
	} else if (!quiet) fprintf(DFILE, "no main output\n");
...
/*	fprintf(DFILE,"dim=%d\n",dim);fflush(DFILE); */
	...
	if (shuffle) {
                if (!quiet) fprintf(DFILE, "reading sites...");
		for (num_sites=0; read_next_site(num_sites); num_sites++);
		if (!quiet) {fprintf(DFILE,"done; num_sites=%ld\n", num_sites);fflush(DFILE);}
		if (!quiet) fprintf(DFILE,"shuffling...");
		init_rand(seed, quiet);
		make_shuffle();
		shuf = &shufflef;
		get_site_n = get_site_offline;
	} else {
                if (!quiet) fprintf(DFILE,"not shuffling\n");
		shuf = &noshuffle;
		get_site_n = read_next_site;
	}

	if (!quiet) fprintf(DFILE, "finding %s...",
		vd ? "Delaunay triangulation" : "convex hull");

	root = build_convex_hull(get_next_site, site_numm, dim, vd);

	fclose(TFILE);
	if (!quiet) {fprintf(DFILE, "done\n"); fflush(DFILE);}
	...
	if (ahull) {
		...
		T = efopen(ahname,"w");
		if (!quiet) {fprintf(DFILE, "finding alpha shape; output to %s\n", ahname); fflush(DFILE);}
	...
	if (vol) {
		...
		if (!quiet) {fprintf(DFILE, "finding volumes; output to %s\n", volnam); fflush(DFILE);}
	...
	if (vd && hist) {
		...
		if (!quiet) {fprintf(DFILE,"finding incidence histograms; output to %s\n", hnam); fflush(DFILE);}
