...
void svg_out(point *v, int vdim, FILE *Fin, int amble) {
  fprintf(stderr, "SVG output is not yet supported\n");
}

void png_out(point *v, int vdim, FILE *Fin, int amble) {
  fprintf(stderr, "PNG output is not yet supported\n");
}

void h_out(point *v, int vdim, FILE *Fin, int amble) {

	static FILE *F;
	static double scaler;

        // fprintf(stderr, "h_out: %p %d %p %d\n", v, vdim, Fin, amble);

	if (Fin) {F=Fin;}

	if (pdim!=2) { warning(-10, "h for planar points only"); return;}

	if (amble==0) {
                // Incremental call - h_out: 0xbfda5b5c 2 (nil) 0
		int i;
		if (!v) return;
		for (i=0;i<vdim;i++) if (v[i]==hull_infinity) {
			point t=v[i];
			...
			break;
                ...
                if (vdim > 1) {
                        if (vdim == 2) {
                                /*
                                fprintf(F, "triangle(%G,%G, %G,%G, %G,%G);\n",
                                        v[0][0]*scaler,v[0][1]*scaler,
                                        v[1][0]*scaler,v[1][1]*scaler,
                                        v[2][0]*scaler,v[2][1]*scaler
                                        );
                                 */
        		        fprintf(F,
	        		        "  edge(%G,%G, %G,%G);\n",
	                		v[0][0]*scaler,v[0][1]*scaler,  // pt0.x,pt0.y
                                        v[1][0]*scaler,v[1][1]*scaler); // pt1.x,pt1.y
                        } else if (vdim == 3) {
        		        fprintf(F,
	        		        "  line3D(%G,%G, %G,%G, %G,%G)\n",
	                		v[0][0]*scaler,v[0][1]*scaler,
                                ...
                                warning(-10, "higher dimensions not supported");
                        }
                }

	} else if (amble==-1) {
                // Initial call - h_out: (nil) 0 0xb7e89d80 -1
		float len[2], maxlen;
		...
		scaler = 216/maxlen;
                ...
                // Final call - h_out: (nil) 0 0xb7e89d80 1
	}
}

void txt_out(point *v, int vdim, FILE *Fin, int amble) {
	...
	if (pdim!=2) { warning(-10, "txt for planar points only"); return;}
